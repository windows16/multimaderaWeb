...
import { getAllPedidosCerrados, getDetallesPedidosCerrados } from "@/features/pedidos/services/pedidos-service"
...
export default function ReporteHistorialPedidos() {
  ...
  const {
    ...
    fetchFn: fetchHistorialPaginados,
    initialLimit: 10,
  })
      ...
      {pedidosFiltrados.length === 0 ? (
        <div className="text-center py-12 space-y-2">
          <p className="text-sm text-muted-foreground">
            {filtrosActivos.length > 0 ? "Ningún pedido coincide con los filtros aplicados." : "No hay pedidos en el historial."}
          </p>
              ...
              <div key={pedido.idPedido} className="border rounded-lg overflow-hidden">
                ...
                <div className="grid grid-cols-3 gap-2 px-4 py-2 bg-muted/30 border-t text-xs">
                  <div>
                    ...
                    <span className="font-medium text-slate-700">{pedido.nombreAlbanil || "No asignado"}</span>
                  </div>
                  <div>
                        ...
                        <tbody>
                          {pedido.detalles.map(d => (
                            <tr key={d.idDetallePedido} className="border-t hover:bg-muted/20 divide-x divide-border">
                              ...
                              <td className="px-4 py-2 text-slate-600">{d.material}</td>
                              <td className="px-4 py-2 font-medium text-right text-slate-700">{fmtMonto(d.total || 0)}</td>
                            </tr>
                          ))}
                          <tr className="border-t">
                            ...
                            <td className="px-4 py-2 font-semibold text-right text-slate-900">{fmtMonto(pedido.subtotal)}</td>
                          </tr>
                        </tbody>
                      </table>
                    )}
